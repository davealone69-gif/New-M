...
function safeRelativeFile(filePath: string) {
  ...
  if (!normalized || normalized.includes('..') || normalized.startsWith('.git/') || normalized.startsWith('gradle/wrapper/')) {
    throw new Error(`Rejected generated file path: ${filePath}`);
  }
  const allowed = /^(app\/src\/main\/(java|res|AndroidManifest\.xml|assets)\/|app\/src\/test\/|app\/proguard-rules\.pro|README\.md$)/;
...
export async function buildAndroidApp(
  generated: { mainActivity: string; extraFiles?: GeneratedFile[] },
): Promise<BuildResult> {
  ...
  try {
    ...
    const args = gradle === 'gradle' ? ['--no-daemon', '--stacktrace', 'assembleDebug'] : ['--no-daemon', '--stacktrace', 'assembleDebug'];
    try {
      const result = await execFileAsync(gradle, args, {
        cwd: root,
