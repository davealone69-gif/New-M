  ...
  build:
    ...
    - name: Build Debug & Release APK Binaries
      run: |
        cd ${'$'}{{ env.BUILD_DIR }}
        chmod +x ./gradlew
        ./gradlew --no-daemon --stacktrace assembleDebug assembleRelease

    - name: Verify APK Integrity & Print SHA-256 Hashes
      run: |
        echo "=== VERIFYING APK ARTIFACTS ==="
        find ${'$'}{{ env.BUILD_DIR }}/app/build/outputs/apk/ -name "*.apk" -exec ls -lh {} +
        find ${'$'}{{ env.BUILD_DIR }}/app/build/outputs/apk/ -name "*.apk" -exec sha256sum {} +

    - name: Upload Release APK Binary
      uses: actions/upload-artifact@v4
      with:
        name: MandelaMatrixOS-Release-APK
        path: ${'$'}{{ env.BUILD_DIR }}/app/build/outputs/apk/release/*.apk

    - name: Upload Debug APK Binary
      uses: actions/upload-artifact@v4
      with:
        name: MandelaMatrixOS-Debug-APK
        path: ${'$'}{{ env.BUILD_DIR }}/app/build/outputs/apk/debug/*.apk`;
...
./gradlew assembleDebug assembleRelease --no-daemon`;
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      ...
      // android-app Root Folder
      const androidAppDir = zip.folder('android-app');
      androidAppDir?.file('settings.gradle.kts', settingsGradleContent);
      androidAppDir?.file('build.gradle.kts', rootBuildGradle);
      androidAppDir?.file('gradle.properties', 'org.gradle.jvmargs=-Xmx2048m\nandroid.useAndroidX=true\n');
      androidAppDir?.file('gradlew', gradlewScript);
      androidAppDir?.file('gradlew.bat', '@rem Gradle wrapper for Windows\ngradle %*\n');

      // Gradle Wrapper Folder
      const wrapperDir = androidAppDir?.folder('gradle')?.folder('wrapper');
      wrapperDir?.file('gradle-wrapper.properties', gradleWrapperProperties);

      // App directory inside android-app
      const appDir = androidAppDir?.folder('app');
